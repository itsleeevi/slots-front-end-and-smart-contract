...
contract Slots is Ownable, VRFConsumerBase {
    ...
    function prizeOf(address player) external view returns(uint256) {
        return prizes[player];
    }
    ...
    function setMultiplier(uint256 first_, uint256 second_, uint256 third_, uint256 fourth_) external onlyOwner()  {
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
        ...
        randomNumbers[0] = (randomness % (maxSymbolValue-minSymbolValue)) + minSymbolValue; // random number between 1 and 6

        // getting 2 more random numbers out of 1 truly random
        for (uint256 i = 1; i < randomNumbers.length; i++) {
            randomNumbers[i] = uint256(keccak256(abi.encode(randomNumbers[0], i))); 
        }
        requestIdToRandomNumbers[requestId] = randomNumbers; 
        Combination memory result = Combination(randomNumbers[0], randomNumbers[1], randomNumbers[2]);
    ...
    function calculatePrize(bytes32 requestId) internal {
        require(requestIdToAddress[requestId] != address(0), "Player cannot be null address.");

        // 6-6-6 
        // --> Pays: 20x
        if (results[requestId].left == 6 && 
            results[requestId].center == 6 && 
            results[requestId].right == 6) {
   
            _LLTH.transfer(msg.sender, bets[requestIdToAddress[requestId]].mul(multiplier.first));           
        }
        // 5-5-5, 4-4-4, 3-3-3, 2-2-2, 1-1-1 
        // --> Pays: 5x
        else if (results[requestId].left == results[requestId].center &&
                 results[requestId].left == results[requestId].right && 
                 results[requestId].left != 6) {

            _LLTH.transfer(msg.sender, bets[requestIdToAddress[requestId]].mul(multiplier.second));
        }
        // 6-6-x, 6-x-6, x-6-6 
        // --> Pays: 4x
        else if ((results[requestId].left == 6 && 
                  results[requestId].center == 6 &&
                  results[requestId].right != 6) || 
                  // ----------------------------
                 (results[requestId].left == 6 && 
            ...
            _LLTH.transfer(msg.sender, bets[requestIdToAddress[requestId]].mul(multiplier.third));   
        }
        // 6-x-x, x-6-x, x-x-6
        // --> Pays: 1x
        else if ((results[requestId].left == 6 && 
            ...
            _LLTH.transfer(msg.sender, bets[requestIdToAddress[requestId]].mul(multiplier.fourth));
        }

        delete bets[requestIdToAddress[requestId]];
